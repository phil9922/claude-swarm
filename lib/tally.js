...
/**
 * Ephemeral working directories created by `evals/run.js`. Benchmark runs spawn
 * real roster agents, so counting them would let the measurement inflate the
 * very number it exists to check. They are not projects; skip them.
 */
const BENCH_SLUG = /^-tmp-cs-bench-/

/** Every project directory Claude Code has recorded, benchmark scratch aside. */
function allProjectSlugs() {
  const root = path.join(configDir(), 'projects')
  return readdirSafe(root)
    .filter((e) => e.isDirectory())
    .map((e) => e.name)
    .filter((name) => !BENCH_SLUG.test(name))
}
